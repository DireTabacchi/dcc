...
void Sema_init(Sema *sa) {
    SymTable_init(&sa->lbl_table);
}

void Sema_deinit(Sema *sa) {
    SymTable_deinit(&sa->lbl_table);
}
...
static void resolve_expression(CompDriver *cd, Expr *expr) {
    ...
    case EXPR_ASSIGN:
        ...
        break;
    case EXPR_VAR: {
        if (SymTable_contains(&cd->parser.syms, expr->as.var->cstr, SYMTYPE_MAPPING)) {
            SymEntry *canon_name =
                SymTable_get(&cd->parser.syms, expr->as.var->cstr, SYMTYPE_MAPPING);
...
static void resolve_declaration(CompDriver *cd, BlockItem *item) {
    ...
    case DECL_LCL_VAR: {
        ...
        if (SymTable_contains(&cd->parser.syms,
        ...
        SymTable_insert_mapping(&cd->parser.syms, var_decl->pos,
